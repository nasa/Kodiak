...
namespace kodiak {
  ...
  class MinMaxSystem : public System, public BranchAndBoundDF<Certainties,MinMax> {
  public: 
    ...
    const MinMax &answer() { return minmax_; }

    void minmax(const Real &, const MinMaxType = MINMAX); 
    void min(const Real &e) { minmax(e,MIN); }
    void max(const Real &e) { minmax(e,MAX); }
    void print(std::ostream & = std::cout) const;

  private:
    void split(const DirVar &, Box &, const Interval &, const real);
    void evaluate(MinMax &, Certainties &, Box &);
    ...
    void select(DirVar &, Certainties &, Box &);
    bool sound(const MinMax &, const Certainties &, const Box &);
