...
namespace kodiak {
    ...
    public:
        ...
        void first_bifurcation() {
            bifurcation(true);
        }

        void equilibrium() {
            bifurcation(false, true);
        }

        void first_equilibrium() {
            bifurcation(true, true);
        }
        void params(const NamedBox &);

        nat param(const std::string id, const Interval &I) {
            return param(id, I, I);
        }
        nat param(const std::string, const Interval &, const Interval &);

        void dfeq(const Real &e) {
            dfeq(e, defaultEnclosureMethodTrueBernsteinFalseInterval_);
        }
