...
RealBinary_Node::RealBinary_Node(const OpType op, const Real &e1, const Real &e2) : 
  ...
  vars_ = e1.vars();
  vars_.insert(e2.vars());
  locals_ = e1.locals();
  locals_.insert(e2.locals().begin(),e2.locals().end());
  consts_ = e1.consts();
  consts_.insert(e2.consts().begin(),e2.consts().end());
  type_ = REAL;
...
RealLetin_Node::RealLetin_Node(const std::string name, const Real &let, const Real &in) : 
  ...
  vars_ = in.vars(); 
  vars_.insert(let.vars());
  locals_ = in.locals();
  locals_.erase(name);
  locals_.insert(let.locals().begin(),let.locals().end()); 
  consts_ = in.consts();
  consts_.insert(let.consts().begin(),let.consts().end());
  NameSet interset;
  intersection(locals_,consts_,interset);
  type_ = REAL;
  if (!interset.empty()) {
...
RealIfnz_Node::RealIfnz_Node(const Real &cond, const Real &lt, const Real &gt) : 
  ...
  vars_ = cond.vars(); 
  vars_.insert(lt.vars());
  vars_.insert(gt.vars());
  locals_ = cond.locals();
  locals_.insert(lt.locals().begin(),lt.locals().end()); 
  locals_.insert(gt.locals().begin(),gt.locals().end()); 
  consts_ = cond.consts();
  consts_.insert(lt.consts().begin(),lt.consts().end()); 
  consts_.insert(gt.consts().begin(),gt.consts().end()); 
  type_ = REAL;
  NameSet interset;
  intersection(locals_,consts_,interset);
  if (!interset.empty()) {
  ...
  maxvar_ = std::max(cond.maxvar(),std::max(lt.maxvar(),gt.maxvar()));
} 
...
Polynomial_Node::Polynomial_Node(const Monomials &monoms, const Names &names) {
  ...
  for (nat j=0; j < nterms(); ++j) {
    monoms_[j] = monoms[j]; 
    vars_.insert(monoms[j].vars());
    locals_.insert(monoms[j].locals().begin(),monoms[j].locals().end());
    consts_.insert(monoms[j].consts().begin(),monoms[j].consts().end());
    if (monoms[j].nvars() > maxvar_)
      maxvar_ = monoms[j].nvars();
  }
  degs_.assign(maxvar_,0);
  for (nat j=0;  j < nterms(); ++j) {
    for (nat i=0; i < maxvar_; ++i) {
...
bool Polynomial_Node::diffConstants(NamedBox &constbox) {
  if (locals_.empty() && consts_.empty())
    return false;
  bool diffconsts = C_.empty();
  if (!diffconsts) {
    for (NameSet::iterator it=locals_.begin(); it != locals_.end(); ++it) {
  ...
  if (!diffconsts) {
    for (NameSet::iterator it=consts_.begin(); it != consts_.end(); ++it) {
      Interval iC = C_.val(C_.var(*it));
      Interval ic = constbox.val(constbox.var(*it));
      if (!iC.seq(ic)) {
  diffconsts = true;
...
Interval Polynomial_Node::eval_bp(const Box &box, NamedBox &constbox) {
  ...
  Interval tmp;
  real wid;
  for (nat i=0; i < nvars(); ++i) {   // for each variable
    if (diff_[i] >= 0) {
      wid = width(box[i]);
      for (nat j=0; j < nterms(); ++j) {   // for each term
  ...
  for (nat ii=0; ii <= degs_[i]; ++ii) {   // for each (univariate) BC
    ...
    for (nat jj=0;jj <= ii && jj <= kk; ++jj) {
      tmp = Interval(Kodiak::choose(kk,jj))*power(box[i].Inf(),kk-jj);
  ...
  if (nvars() >= 3) {  // only use this version for non-trivial degree
    ...
    blocksize.resize(nvars());
    blocksize[0] = 1;
    for (nat i=1; i < nvars(); ++i)
  ...
  //        if (monos_[j].expo(i)!=0 && tmbsize[i]!=1) {
  ...
  } else { // old version - should be used where nvars is <= 2?
    ...
    for (nat i=0; i < b_min_size || i < b_max_size; ++i) { // for each BC to compute
      ...
      for (nat ii=0; ii < nvars(); ++ii) { // for each variable
...
Rational_Node::Rational_Node(const Monomials &nummonoms, const Monomials &denmonoms, const Names &names)
{
    ...
    for (nat j = 0; j < numnterms(); ++j)
    {
        ...
        vars_.insert(nummonoms[j].vars());
        locals_.insert(nummonoms[j].locals().begin(), nummonoms[j].locals().end());
        consts_.insert(nummonoms[j].consts().begin(), nummonoms[j].consts().end());
        if (nummonoms[j].nvars() > maxvar_)
            maxvar_ = nummonoms[j].nvars();
    }
    for (nat j = 0; j < dennterms(); ++j)
    {
        den_monoms_[j] = denmonoms[j];
        vars_.insert(denmonoms[j].vars());
        locals_.insert(denmonoms[j].locals().begin(), denmonoms[j].locals().end());
        consts_.insert(denmonoms[j].consts().begin(), denmonoms[j].consts().end());
        if (denmonoms[j].nvars() > maxvar_)
            maxvar_ = denmonoms[j].nvars();
    }
    degs_.assign(maxvar_, 0);
    for (nat j = 0;  j < numnterms(); ++j)
    {
...
Interval Rational_Node::eval_bp(const Box &box, NamedBox &constbox)
{
    ...
    if (btd.ple(0)) densign = -1; else densign = 1;
    bmax = bmin = btn / btd;
