...
namespace kodiak {
  ...
  class Node {
    friend class Real;
  protected:
    Node() : maxvar_(0), use_(0) {}
    virtual bool isVal() const { return false; }
    ...
    virtual bool isLetin() const { return false; }
    virtual bool isIfnz() const { return false; }
    virtual bool isPolynomial() const { return false; }
    virtual bool isRational() const { return false; }
    virtual Real deriv(const nat) const { return NotAReal; }
    ...
    VarBag   vars_;   // variable indices (nats)
    nat      maxvar_; // index of maximum variable (+1)
    NameSet  locals_; // local variables (names)
    NameSet  consts_; // global consts (names)
    RealType type_;   // type of real expression 
  private:
    int use_;
  };
  ...
  public:
    ...
    bool isUnary() const { return true; }
    Real deriv(const nat) const;
    Interval eval(const Box &, NamedBox &, const bool);
    Real subs(const Substitution &, Names &) const;
    void print(std::ostream & = std::cout) const;
  ...
  public:
    ...
    bool isPower() const { return true; }
    Real deriv(const nat) const;
    Interval eval(const Box &, NamedBox &, const bool);
    Real subs(const Substitution &, Names &) const;
    void print(std::ostream & = std::cout) const;
  ...
  public:
    ...
    bool isLetin() const { return true; }
    Real deriv(const nat) const;
    Interval eval(const Box &, NamedBox &, const bool);
    Real subs(const Substitution &, Names &) const;
    void print(std::ostream & = std::cout) const;
  ...
  enum MonoInfo {UNK, INC, DEC, CNST};
  ...
  public:
    ...
    Real subs(const Substitution &, Names &) const;
    void print(std::ostream & = std::cout) const;
    nat nterms() const { return monoms_.size(); } // number of monomials
    void print_metadata(std::ostream & = std::cout) const;
  private:
    Names names_;
    nat nvars() const { return maxvar_; }  // number of variables
...
class Rational_Node : public Node {
  ...
  public:
    ...
    void print(std::ostream & = std::cout) const;
    void print_metadata(std::ostream & = std::cout) const;
    nat numnterms() const { return num_monoms_.size(); }
    nat dennterms() const { return den_monoms_.size(); }
  private:
    Names names_;
    nat nvars() const { return maxvar_; }  // number of variables
    nat degree(const nat) const; // degree in a variable
