...
public:
  ...
  bool find(T t) const { return bag_.find(t) != bag_.end(); }
  nat count(T t) const {
  ...
  iterator begin() const { return bag_.begin(); }
  iterator end() const { return bag_.end(); }
...
public:
  ...
  bool next();
  nat  size() const { return perm_.size(); }
  int  sign() { return (even_ ? 1 : -1); }
  const Tuple &perm() const { return perm_; }
  const nat &operator[](nat i) const { return perm_[i]; }
...
public:
  ...
  Interval(const real val) : filib::interval<real>(val) { check_input(val); }
  Interval(const real lb, const real ub) : filib::interval<real>(lb, ub) {}
  Interval Mid() const { real mid = this->mid(); return Interval(mid, mid); }
  Interval Inf() const { real inf = this->inf(); return Interval(inf, inf); }
  Interval Sup() const { real sup = this-> sup(); return Interval(sup, sup); }
  static Interval Empty() { return Interval(1, -1); }
  bool empty() const { return !contains(inf()); }
...
public:
  ...
  // Push a new variable
  nat push(const std::string id) { return push(id, 0); }
  nat push(const std::string, const Interval &);
  nat push(const std::string, const Interval &, const Interval &);
  nat push(const Interval &val) { return push("", val); };
  nat push(const Interval &lb, const Interval &ub) { return push("", lb, ub); };
  ...
  void pop() { names_.pop_back(); box_.pop_back(); }
  ...
  void clear() { names_.clear(); box_.clear(); }
  nat size() const { return box_.size(); }
  void resize(nat v) { box_.resize(v); names_.resize(v); }
  nat  var(const std::string) const;
  Interval val(const nat v) const { return box_[v]; }
  std::string name(const nat) const;
  const Box &box() const { return box_; }
  const Names &names() const { return names_; }
  Box &box() { return box_; }
  Names &names() { return names_; }
...
public:
  static void init();
  static nat precision() { return precision_; }
  static void set_precision(const nat = K_PRECISION);
  static std::string var_prefix() { return var_prefix_; }
  static void set_var_prefix(const std::string s = K_PREFIX) { var_prefix_ = s; }
  static bool safe_input() { return safe_input_; }
  static void set_safe_input(const bool b = true) { safe_input_ = b; }
  static bool debug() { return debug_; }
  static void set_debug(const bool b = true) { debug_ = b; }
...
struct DirVar {
  ...
  void init(nat v) { dir = true; var = v; onlyone = false; splitting = 0; first_ = true; }
  void next() { dir = !dir; first_ = !first_; }
...
enum OpType { NOP, ADD, SUB, MULT, DIV, NEG, ABS, SQ, SQRT, EXP, LN, SIN,
...
enum Where { EXTERIOR, LEFT_INTERIOR, RIGHT_INTERIOR, INTERIOR };
