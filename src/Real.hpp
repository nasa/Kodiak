...
namespace kodiak {
  ...
  enum RealType { ATOM, MONOMEXPR, POLYNOMIAL, POLYEXPR, RATIONAL, REAL };
  ...
  public:
    ...
    bool notAReal() const { return node_ == NULL; }
    bool isVal() const;
    ...
    void checkIndexed() const { Defs defs; checkIndexed(defs); }
    void checkIndexed(Defs &) const;

    Interval eval(const Box &box = EmptyBox, const bool enclosure = false) const { 
      NamedBox constbox; return eval(box,constbox,enclosure); 
    }
    ...
    Interval eval(const NamedBox &varbox, const bool enclosure = false) const { 
      NamedBox constbox; return eval(varbox,constbox,enclosure); 
    }

    Interval enclosure(const Box &box) const { return eval(box,true); }
    Interval enclosure(const NamedBox &varbox) const { return eval(varbox,true); }
    Interval enclosure(const NamedBox &varbox, NamedBox &constbox) const { 
      return eval(varbox,constbox,true); 
    }
    Real copy() const { Substitution sigma; return subs(sigma); }
    Real subs(const Substitution &, Names &) const;
    Real subs(const Substitution &sigma) const { Names names; return subs(sigma,names); }
    void print(std::ostream &) const;
    const VarBag &vars() const;
    VarBag &vars();
    bool isFresh(const nat v) const { return !vars().find(v); }
    bool hasVar(const nat v) const { return vars().find(v); }
    // Maxium variable index (+1)
    nat maxvar() const;
    ...
    NameSet &consts();
    RealType type() const;
    bool closed() const { return locals().empty(); } // Expression is closed, no local variables
    bool open() const { return !locals().empty(); } // Expression is open, there are local variables
    ~Real ();
...
// (i,j)-th coefficient of symbolic Hurwitz matrix of dimension n x n
  ...
  public:
    ...
    // accessor functions
    void neg() { coeff_ = -coeff_; }
    void add(Real e, const int sign = 1) { coeff_ = coeff_ + sign*e; }
    nat nvars() const { return expo_.size(); } // number of variables
    nat expo(const nat) const;  // exponent of given variable
    const Real &coeff() const { return coeff_; } // coefficient
    ...
    const Tuple &exponents() const { return expo_; }
    Tuple &exponents() { return expo_; }
    const VarBag &vars() const { return vars_; }
    const NameSet &locals() const { return locals_; }
    const NameSet &consts() const { return consts_; }
  private:
