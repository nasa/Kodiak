...
bool MinMaxSystem::sound(const MinMax &minmax, const Certainties &, const Box &box) {
  Box b = box;
  int cert = evalSystem(b);
  return (cert == 0 || !minmax.empty()) && minmax.sound() &&
    (cert <= 0 ||
...
void MinMaxSystem::minmax(const Real &e, const MinMaxType min_or_max) {
  ...
  temp_.resize(varbox_.size());
  expr_ = e.index(varbox_,constbox_);
  expr_.checkIndexed(defs_);
  dexpr_.resize(varbox_.size());
  for (nat v=0; v < varbox_.size(); ++v) 
    dexpr_[v] = (bp_ ? polynomial(expr_.deriv(v)) : expr_.deriv(v)).subs(defdx_);
  if (bp_) 
    expr_ = polynomial(expr_);
  minmax_.init(min_or_max_.back());
  Certainties certs;
  certs.init(nrels());
  bandb(minmax_,certs,varbox_.box()); 
}

// dirvar.splitting = 0: normal split
// dirvar.splitting = 1: lower and upper bound splits
void MinMaxSystem::split(const DirVar &dirvar, Box &box, const Interval &i, const real mid) {
  if (dirvar.splitting == 0) {
    ...
    if (dirvar.dir) 
      box[dirvar.var] = i.Inf();
    else    
      box[dirvar.var] = i.Sup(); 
...
void MinMaxSystem::evaluate(MinMax &answer, Certainties &certs, Box &box) {
  if (debug() > 1) {
  ...
  int cert = evalSystem(box,certs,dirvars());
  defbox = defbox_;
  if (debug() > 1)
    std::cout << "Box: " << box << ", Certs: " << certs << std::endl; 
  ...
  if (mid_cert > 0) { // Midpoint satisfies constraints
    Interval Mid = expr_.eval(temp_,defbox_);
  ...
  for (VarBag::iterator it=expr_.vars().begin(); it!=expr_.vars().end(); ++it) {
    nat v = it->first;
    if (!box[v].isPoint() && box[v].diam() > resolutions_[v]) {
      Interval save_it = temp_[v];
      temp_[v] = box[v].Inf();
      int inf_cert = evalSystem(temp_);
      Interval lb_it = expr_.eval(temp_,defbox_);
	...
	if (answer.max_point_.empty() || lb_it.inf() > answer.lb_of_max_) {
      ...
      Interval ub_it = expr_.eval(temp_,defbox_);
	...
	if (answer.max_point_.empty() || ub_it.inf() > answer.lb_of_max_) {
      ...
      if (cert > 0 ) {
	try {
	  d_it = dexpr_[v].eval(box,defbox,bp_);
	  bool ge = d_it.cge(0);
	  bool le = d_it.cle(0);
	  if (ge || le) {
	    dirvar_.dir = true;
  ...
  answer.mm_ = expr_.eval(box,defbox,bp_);
  if (var_min < box.size()) {
...
bool MinMaxSystem::local_exit(const MinMax &answer) {
  bool b = answer.empty() ||
    (min_or_max_.back() > 0 || 
     ...
     answer.ub_of_min_ - answer.mm_.inf() <= tolerance_) &&
    (min_or_max_.back() < 0 || 
     !answer.max_point_.empty() &&
     answer.mm_.sup() - answer.lb_of_max_ <= tolerance_);
  return b;
}

void MinMaxSystem::select(DirVar &dirvar, Certainties &certs, Box &box) { 
  if (varselect_ == 0)
...
void MinMaxSystem::print(std::ostream &os) const {
  ...
  if (debug() > 0) {
    for (nat v=0; v < theBox().size(); ++v) {
      os << "  d" << id() << "/d" << varbox_.name(v) << " = " 
	 << dexpr_[v] << std::endl;
    }
  }
