...
{
    if (!notAReal())
        return node_->locals_;
    throw Growl("Kodiak (locals): set of local variables of NotAReal is undefined");
}
...
nat Real::maxvar() const
{
    if (!notAReal())
        return node_->maxvar_;
    throw Growl("Kodiak (maxvar): maximum variable index of NotAReal is undefined");
}
...
{
    return !notAReal() &&
           node_->type_ <= MONOMEXPR;
}

bool Real::isPolyExpr() const
{
    return !notAReal() &&
           node_->type_ <= POLYEXPR;
}
...
Real Real::subs(const Substitution &sigma, Names &names) const
{
    ...
    throw Growl("Kodiak (subs): substitution of NotAReal is undefined");
}
...
std::ostream &kodiak::operator<< (std::ostream &os, const RealType t)
{
    switch (t)
    {
    case ATOM: os << "ATOM"; break;
    case MONOMEXPR: os << "MONOMEXPR"; break;
    case POLYNOMIAL: os <<  "POLYNOMIAL"; break;
    case POLYEXPR: os << "POLYEXPR"; break;
    case RATIONAL: os <<  "RATIONAL"; break;
